...
describe('journal T1 crash recovery integration', () => {
  ...
  beforeEach(async () => {
    ...
    // Initialize project
    const initResult = JSON.parse(await projectInitCmd(['recovery-test-project', 'Test crash recovery']));
  ...
  it('recovers correctly after crash mid-wave-1', async () => {
    ...
    const checkpointPath = path.join(tmpDir, '.tff', 'milestones', 'M01', 'slices', 'M01-S01', 'CHECKPOINT.md');
