...
- **S09 — Skills Architecture Reform**: All 18 skills restructured from `skills/<name>.md` to `skills/<name>/SKILL.md` folder convention; all skills decoupled from TFF-specific commands, paths, agent names, ∧ tier terminology (S-tier/F-lite/F-full → simple/standard/complex); `token-budget` frontmatter removed; hexagonal-architecture fully rewritten with DDD + CQRS patterns; all references updated across workflows, agents, README, ∧ integration tests
- **Agent count**: 13 → 4 (lean identity-only agents; methodology now ∈ skills)
...
- **R02 — S-tier criteria:** S-tier restricted to single-file changes, ¬ new files, ¬ architecture impact, root cause known (S01)
- **R03 — User-confirmed classification:** `slice:classify` output is a suggestion, ¬ auto-routing; ¬ workflow step auto-skipped (S02, S03)
- **R04 — Unified pipeline:** All tiers follow same pipeline (discuss → research → plan → execute → verify → ship); tiers control depth, ¬ skipping (S03)
- **R05 — Quick/debug alignment:** Quick ∧ debug become entry-point shortcuts that converge on standard plan → execute → ship (S04)
- **Removed `.tff/` from git tracking** — internal project state is ¬ longer version-controlled

### Fixed

- **R06 — Milestone numbering:** Fixed to use `--all` flag for closed beads ∈ numbering scans (S05)

## [0.5.3] - 2026-03-22

### Added

- Bead-vs-PR reconciliation ∈ `/tff:health` — detects stale beads whose PRs are already merged
- Error handling convention (rule 6) ∈ orchestrator pattern — all tff-tools calls must check `ok` field
- Explicit error checks after state transitions ∈ discuss, research, plan, execute, verify workflows
- Plan Review column ∈ complexity tiers — plannotator review mandatory for ALL tiers including S-tier
...
- `quick.md` ∧ `debug.md` now include plannotator-annotate plan review step (was skipped)
...
- Path traversal guard ∈ `MarkdownArtifactAdapter.resolve()` — rejects `../` escape attempts
- Dolt remote validation — `doltPush`/`doltPull` reject non-alphanumeric remote names (prevents flag injection)

### Security

- Path traversal: resolved path must start with basePath (defense-∈-depth)
- Dolt remote: validates against `^[a-zA-Z0-9_-]+# Changelog
...
- Path traversal: resolved path must start with basePath (defense-∈-depth)
- Dolt remote: validates against  before `execFile`

## [0.5.2] - 2026-03-22

### Changed

- Project init (`/tff:new`) ¬ longer creates requirements ∨ first milestone — just PROJECT.md + settings.yaml, then suggests `/tff:new-milestone`
...
**CLI Tooling (R01)**
- S01: Corrected misused error codes ∧ version drift across CLI commands
...
- S14: Parse `--title` flag ∧ skip unknown flags ∈ `slice:create` (was treating `--milestone` as slice name)

**Dual-State Coordination (R03)**
- S03: Fixed `InMemoryBeadStore.ready()` dependency resolution ∧ typed dolt-sync
- S06: Read actual bead status ∈ `slice:transition` instead of stale cached value
...
- S09: Enforce `.tff/worktrees/` path ∈ quick ∧ debug workflows with explicit `tff-tools worktree:create` calls
- S11: Make auto-transition instructions explicit ∈ all workflows (read settings, check autonomy mode)

**Observability (R06)**
- S02: Added `tffWarn` helper ∧ surfaced swallowed errors ∈ beads adapter
- S13: Fixed PostToolUse hook path resolution — use `${CLAUDE_PLUGIN_ROOT}` instead of relative path

**Dual-State Persistence (R03)**
- S10: Auto-generate STATE.md ∧ save checkpoints after slice transitions

**Documentation**
- S05: Fixed Dolt remote setup instructions ∈ new-project workflow

**Security**
- Removed `.beads/.beads-credential-key` from git tracking ∧ added to `.gitignore`
...
- `debugging-methodology` skill — Track A (reproducible error) ∧ Track B (symptom-based) diagnostic methodology
- Phase 1 (diagnose) runs without creating a slice; Phase 2 creates S-tier slice only after root cause confirmed
- External root cause detection — exits with diagnostic report ∧ workaround suggestions instead of entering fix phase
...
- Synthesizes proposed project name, vision, ∧ requirements from generated docs
...
- Dolt auto-sync ∈ `slice-transition.cmd.ts` replaced naive `includes('auto-sync: true')` substring matching with proper parsed settings — commented-out dolt config ¬ longer falsely triggers sync
...
- Field-level conflict resolution: status (latest ts), design (flag conflict), deps (∪), kvs (latest ts)
- `CONFLICT.md` generated for design conflicts requiring human resolution
- Clone-∧-go: `/tff:init` hydrates beads from existing snapshot
- Dolt remote sync utilities (`doltPush`, `doltPull`, `shouldAutoSync`)
- Auto-push to Dolt remote on transitions when configured
- Guided Dolt remote setup during `/tff:new` (DoltHub, self-hosted, ∨ skip)
- Git merge driver setup ∈ `/tff:new` workflow
...
- New `interactive-design` skill -- conversation methodology, spec templates (F-lite/F-full), spec-document-reviewer ∧ plan-document-reviewer prompt templates
...
- Marketplace.json: removed `version` field (¬ needed for CC plugin updates, caused stale cache)

## [0.3.1] - 2026-03-21

### Fixed

- Removed `version` from marketplace.json (¬ needed for CC plugin updates, caused stale cache issues)
...
**Agent Intelligence**
- All 13 agents enriched with **Personality**, **Methodology**, ∧ **Reads Before Acting** sections
- `tff-skill-drafter` agent (13th agent, added ∈ 0.2.0) now has full personality profile
- `references/security-baseline.md` -- STRIDE threat categories ∧ OWASP Top 10 quick reference for security-auditor

**Smarter Context Management**
- Token budget tiers (`critical` / `workflow` / `background`) on all 5 skill files
- Workflow SPAWN instructions rewritten to pass role-specific context (¬ full slice context)

**Auto-Learn Pipeline Hardening**
- Refinement cooldown metadata (`canRefine`, `recordRefinement`) with configurable 7-day cooldown
- Configurable scoring weights ∈ `rankCandidates` (frequency, breadth, recency, consistency)
...
- `autonomy.mode` setting: `guided` (V2 behavior) ∨ `plan-to-pr` (auto-run from plan approval to PR)
- Workflow chaining: `nextWorkflow()` ∧ `shouldAutoTransition()` use cases
...
- Sort determinism documented ∈ wave detection

**Test Coverage**
- 384 tests across 92 test files (up from ~300 ∈ 0.2.0)
- Property-based tests for scoring (fast-check)
- Sync edge case tests (∅, partial, large orphan sets)
...
- Configurable `minCount` ∈ `aggregatePatterns` (was hardcoded default only)
...
- PR URLs always shown to user ∈ ship-slice ∧ complete-milestone workflows
...
- Entity IDs changed from `z.uuid()` to `z.string().min(1)` (beads uses hash IDs, ¬ UUIDs)
- `milestone:create` auto-detects project bead ID ∧ auto-numbers milestones
- `slice:create` auto-detects active milestone ∧ auto-numbers slices
...
- Hooks rewritten: removed Node.js hooks (caused PostToolUse/SessionStart errors), ∅ hooks.json
- Fixed label inheritance pollution (`--no-inherit-labels` ∈ `bd create`)
- Fixed `normalizeBeadData` to find first `tff:` label ∈ labels array
...
- Context monitor hook (caused errors, ¬ bridge file writer ∃)
...
- `enforceFreshReviewer` -- reviewer must ¬ be executor
...
- ∈-memory test adapters for all ports
