...
export const sliceCreateCmd = async (args) => {
    ...
    return withBranchGuard(async ({ milestoneStore, sliceStore }) => {
        ...
        const milestone = openMilestones.length > 0
            ? openMilestones[openMilestones.length - 1]
            : milestonesResult.data[milestonesResult.data.length - 1];
        const milestoneId = milestone.id;
        const result = await createSliceUseCase({ milestoneId, title: name }, { milestoneStore, sliceStore, artifactStore, stateBranch });
        if (isOk(result))
            return JSON.stringify({ ok: true, data: result.data });
        return JSON.stringify({ ok: false, error: result.error });
    });
};
//# sourceMappingURL=slice-create.cmd.js.map