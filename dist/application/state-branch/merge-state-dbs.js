 ...
 */
export function mergeStateDbs(parentDbPath, childDbPath, sliceId) {
    try {
        ...
        const sliceR = parentDb
            .prepare(`INSERT OR REPLACE INTO slice (id, milestone_id, number, title, status, tier, created_at, updated_at)
        ...
        const taskR = parentDb
            .prepare(`INSERT OR REPLACE INTO task (id, slice_id, number, title, description, status, wave, claimed_at, claimed_by, closed_reason, created_at, updated_at)
        ...
        parentDb
            ...
            .run(sliceId);
        const depR = parentDb
            .prepare(`INSERT INTO dependency (from_id, to_id, type)
...
//# sourceMappingURL=merge-state-dbs.js.map