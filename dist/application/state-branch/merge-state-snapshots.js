 ...
 */
export function mergeStateSnapshots(parent, child, sliceId) {
    try {
        // Validate that child has the specified slice
        const childSlice = child.slices.find((s) => s.id === sliceId);
        if (!childSlice) {
            return Err(syncFailedError(`Child snapshot does not contain slice "${sliceId}"`, {
                sliceId,
        ...
        return Err(syncFailedError(`Snapshot merge failed: ${e instanceof Error ? e.message : String(e)}`, {
            sliceId,
        }));
    }
}
//# sourceMappingURL=merge-state-snapshots.js.map