...
const ModelProfileSchema = withDefault(z.object({
    model: z.string().catch("sonnet"),
}));
const ModelProfilesSchema = withDefault(z.object({
 ...
 */
export function loadProjectSettings(yamlContent) {
    if (!yamlContent?.trim()) {
        return ProjectSettingsSchema.parse(undefined);
    }
    try {
