...
describe("sync-lock integration", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = join(
			os.tmpdir(),
			`tff-sync-lock-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		...
		const initResult = JSON.parse(
			await projectInitCmd(["test-project", "Test project for sync lock"]),
		);
	...
	describe("concurrent sync:branch calls", () => {
		...
		it("processes sync:branch when lock is available", async () => {
			...
			if (
				result.ok === true &&
				result.data?.action === "skipped" &&
				result.data?.reason?.includes("Lock")
			) {
