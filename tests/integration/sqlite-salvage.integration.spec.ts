...
describe("sqlite-salvage integration", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = join(
			os.tmpdir(),
			`tff-salvage-int-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		...
		const initResult = JSON.parse(
			await projectInitCmd(["salvage-test", "Test salvage integration"]),
		);
		...
		it("should extract maximum readable data from partially corrupted database", async () => {
			...
			if (isOk(result)) {
				...
				if (metadata.integrityCheckResult !== "ok") {
					expect(
						metadata.corruptionNotes.some((n) => n.includes("Integrity") || n.includes("Quick")),
					).toBe(true);
				}
			}
		}, 10000);
	...
	describe("invalid records filtering", () => {
      ...
      `);
			...
			expect(
				metadata.corruptionNotes.some(
					(n) => n.includes("M02") || n.includes("missing") || n.includes("invalid"),
				),
			).toBe(true);
			expect(
				metadata.corruptionNotes.some(
					(n) => n.includes("M01-S02") || n.includes("Missing Milestone"),
				),
			).toBe(true);
