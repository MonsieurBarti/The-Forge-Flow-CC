...
describe("T2 salvage integration", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = join(
			os.tmpdir(),
			`tff-t2-salvage-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		...
		const initResult = JSON.parse(
			await projectInitCmd(["t2-salvage-test", "Test T2 salvage recovery"]),
		);
      ...
      `);
			...
			db.prepare(
				"INSERT OR REPLACE INTO slice VALUES ('M01-S01', 'M01', 1, 'Test Slice', 'discussing', NULL, ?)",
			).run(now);
			db.prepare(
		...
		const branchLog = execSync("git log --oneline -1", {
			cwd: tmpDir,
			encoding: "utf8",
			env,
		}).trim();
