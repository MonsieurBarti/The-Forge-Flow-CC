...
describe("T3 recovery integration", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = join(
			os.tmpdir(),
			`tff-t3-recovery-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		...
		execSync("git add -A", { cwd: tmpDir, stdio: "pipe", env });
		execSync('git commit -m "Pre-state-branch commit" || true', {
			cwd: tmpDir,
			stdio: "pipe",
			env,
		});
	...
	it("restores .tff/ and regenerates project files when both are missing", async () => {
		...
		const repairResult = JSON.parse(
			await stateRepairCmd(["main", "--tier", "T3", "--milestone", "M01"]),
		);
	...
	it("attempts fetch when state branch is missing locally", async () => {
		...
		execSync("git branch -m tff-state/main tff-state/remote-main", {
			cwd: tmpDir,
			stdio: "pipe",
			env,
		});
