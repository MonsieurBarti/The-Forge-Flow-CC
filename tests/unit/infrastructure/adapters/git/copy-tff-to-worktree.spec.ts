...
import { copyTffToWorktree } from "../../../../../src/infrastructure/adapters/git/copy-tff-to-worktree.js";
...
describe("copyTffToWorktree", () => {
	...
	beforeEach(() => {
		tmpDir = path.join(
			os.tmpdir(),
			`tff-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
	...
	it("copies subdirectories recursively", () => {
		...
		copyTffToWorktree(tffDir, worktreePath);
		expect(
			existsSync(path.join(worktreePath, ".tff", "milestones", "M01", "REQUIREMENTS.md")),
		).toBe(true);
	});
