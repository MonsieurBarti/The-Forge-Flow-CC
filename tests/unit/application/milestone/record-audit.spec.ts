...
describe("recordAuditUseCase", () => {
  it("persists verdict via store with frozen timestamp", async () => {
    ...
    const res = await recordAuditUseCase(
      ...
      { milestoneAuditStore: store as unknown as import("../../../../src/domain/ports/milestone-audit-store.port.js").MilestoneAuditStore, now: () => new Date("2026-04-19T12:00:00Z") },
    );
  ...
  it("propagates store error", async () => {
    ...
    const res = await recordAuditUseCase(
      ...
      { milestoneAuditStore: store as unknown as import("../../../../src/domain/ports/milestone-audit-store.port.js").MilestoneAuditStore },
    );
