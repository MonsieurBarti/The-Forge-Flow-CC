...
async function handleMismatch(error: BranchMismatchError): Promise<boolean> {
	...
	const result = await restoreBranchUseCase(
		{ codeBranch: error.currentBranch, targetDir: cwd },
		{ stateBranch },
	);
	...
	if (isOk(result) && result.data !== null) {
		...
		const metaBufR = await gitOps.extractFile(
			`tff-state/${error.currentBranch}`,
			"branch-meta.json",
		);
		if (isOk(metaBufR)) {
			try {
