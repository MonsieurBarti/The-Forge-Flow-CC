		...
		if (arg === "--tier") {
			...
			if (!nextArg || nextArg.startsWith("--")) {
				return {
					error: {
						code: "INVALID_ARGS",
						message: "Usage: --tier requires a value (T1, T2, or T3)",
					},
				};
			}
			...
			if (tierValue !== "T1" && tierValue !== "T2" && tierValue !== "T3") {
				return {
					error: {
						code: "INVALID_ARGS",
						message: `Invalid tier "${nextArg}". Valid values: T1, T2, T3`,
					},
				};
			}
			...
			if (!nextArg || nextArg.startsWith("--")) {
				return {
					error: {
						code: "INVALID_ARGS",
						message: "Usage: --milestone requires a value (e.g., M001)",
					},
				};
			}
			milestoneId = nextArg;
...
async function validateRestoredState(
	...
	if (sliceIds.length === 0) {
		...
		return {
			consistent: existsSync(stateDbPath) && isDbValid(stateDbPath),
			checkedSlices: 0,
			errors: [],
		};
	}
...
export const stateRepairCmd = async (args: string[]): Promise<string> => {
	...
	if (!codeBranch) {
		return JSON.stringify({
			...
			error: {
				code: "INVALID_ARGS",
				message: "Usage: state:repair <branch> [--tier T1|T2|T3] [--milestone MXXX]",
			},
		});
	}
	...
	if (tier === "T3") {
		...
		type _T3ErrorResult = { code: string; message: string };
		...
		try {
			...
			const restoreResult = await restoreBranchUseCase(
				{ codeBranch, targetDir: cwd },
				{ stateBranch },
			);
	...
	if (tier === "T2") {
		...
		type _T2ErrorResult = { code: string; message: string };
		...
		try {
			...
			if (salvageResult?.ok && salvageResult.data.snapshot && restoreResult.data) {
				try {
					...
					const { SQLiteStateExporter } = await import(
						"../../infrastructure/adapters/export/sqlite-state-exporter.js"
					);
					...
					if (!exportResult.ok) {
						console.warn(
							`[state:repair] Failed to export restored state for merge: ${exportResult.error.message}`,
						);
						adapter.close();
					} else {
						...
						if (!importResult.ok) {
							console.warn(
								`[state:repair] Failed to import merged snapshot: ${importResult.error.message}`,
							);
						} else {
