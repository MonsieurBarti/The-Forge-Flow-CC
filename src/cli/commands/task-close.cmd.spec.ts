...
import { milestoneCreateCmd } from './milestone-create.cmd.js';
import { projectInitCmd } from './project-init.cmd.js';
import { sliceCreateCmd } from './slice-create.cmd.js';
import { taskClaimCmd } from './task-claim.cmd.js';
import { taskCloseCmd } from './task-close.cmd.js';
...
describe('task:close — journal integration', () => {
  ...
  it('accepts optional reason parameter', async () => {
    // Close with reason
    const result = JSON.parse(await taskCloseCmd(['M01-S01-T01', '--reason', 'Completed successfully']));
    expect(result.ok).toBe(true);
  ...
  it('preserves wave index from task in journal entry', async () => {
    ...
    const completedEntry = entries.find((e) => e.type === 'task-completed' && e.taskId === 'M01-S01-T02');
