...
export class GitStateBranchAdapter implements StateBranchPort {
				...
				try {
					...
					if (isOk(importR)) {
						...
						for (const filePath of filesR.data) {
							...
							if (
								!resolved.startsWith(resolvedTargetDir + path.sep) &&
	...
	async merge(
		...
		const parentSnapshotR = await this.gitOps.extractFile(
			parentStateBr,
		...
		const childTaskIds = new Set(
			childSnapshot.tasks.filter((t) => t.sliceId === sliceId).map((t) => t.id),
		);
